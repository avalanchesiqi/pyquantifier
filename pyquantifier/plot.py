...
def plot_empirical_bar(data, **kwds):
    ...
    label_axis = list(data.keys())
    prevalence_axis = [data[k] for k in label_axis]

    if num_label == 2:
        color_palette = ColorPalette.CC2

    ax.bar(x_axis, prevalence_axis, width=0.7, color=color_palette,
           lw=2, edgecolor='k')
    ...
    for k, v in zip(x_axis, prevalence_axis):
